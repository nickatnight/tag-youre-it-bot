...
const restProvider = (apiUrl: string, httpClient = fetchUtils.fetchJson): DataProvider => ({
    getList: (resource, params) => {
        ...
        return httpClient(url).then(({ headers, json }) => {
            ...
            return {
                // data: json.data,
                data: json.data.map(resource => ({ ...resource, id: resource.ref_id }) ),
                total: parseInt(
                    headers.get('x-content-range').split('/').pop(),
                    10
                ),
            };
        });
